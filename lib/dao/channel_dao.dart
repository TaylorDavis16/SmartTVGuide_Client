...
class ChannelDao {
  ...
  static Future<HomeModel> homeData({bool swiper = false, int pageIndex = 1, int pageSize = 10}) async {
    HomeRequest request = HomeRequest();
    request.add('name', 'home').add('swiper', swiper).add("pageIndex", pageIndex).add("pageSize", pageSize);
    var result = await HiNet().fire(request);
    if(result['code'] != 1) throw HiNetError(result['code'], result['message']);
